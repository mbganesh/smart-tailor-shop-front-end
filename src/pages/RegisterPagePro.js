...
import { Button, Stack, Typography, Paper, TextField, IconButton, InputAdornment, Box } from '@mui/material';
...
//   allFieldNames.every(isNotFill)
  ...
  const onChangeEmail = (e) => {
    setRegisterValues((prevStyle) => ({
      ...prevStyle,
      email: { ...prevStyle.email, text: e.target.value, errorMessage: "", isValid: false },
    }));
  ...
  const onChangeMobno = (e) => {
    setRegisterValues((prevStyle) => ({
      ...prevStyle,
      mobno: { ...prevStyle.mobno, text: e.target.value, isValid: false, errorMessage: "" },
    }));
  ...
  const onChangeshopAddress = (e) => {
    ...
    if (registerValues.shopaddress.text) {
      ...
      setRegisterValues((prevStyle) => ({
        ...prevStyle,
        shopaddress: { ...prevStyle.shopaddress, isValid: false, errorMessage: "" },
      }));
    }
  }
        ...
        if (splittedEmail !== "gmail.com") {
          setRegisterValues((prevStyle) => ({
            ...
            [item]: {
              ...prevStyle[item],
              isValid: true,
              errorMessage: "Please Enter Your Gmail Only. Supports only Gmail Accounts ",
            },
          }));
          checkPost = false
          break
  ...
  const onRegisterBtnClick = async () => {
    ...
    if (isAllFieldEntered) {
      ...
      axios.post(APIClient.API_BASE_URL + "/registrationProcess/register", registrationData).then((res) => {
 ...
 <TextField autoComplete="off" fullWidth size="small" label="Name" variant="outlined"
            ...
            <TextField fullWidth size="small" label="Mobile No" variant="outlined" inputProps={{ maxLength: 10 }} onChange={onChangeMobno} value={registerValues.mobno.text}
              autoComplete="off"
            ...
            <TextField fullWidth size="small" label="Password" variant="outlined"
              type={registerValues.showPassword ? 'text' : 'password'} value={registerValues.password.text}
              autoComplete="off"
...
<TextField fullWidth size="small" label="Shop Name" variant="outlined" onChange={onChangeShopName} value={registerValues.shopname.text}
              ...
              <TextField fullWidth size="small" label="Shop Phone Number" variant="outlined" onChange={onChangeShopMobNo} value={registerValues.shopmobno.text}
              autoComplete="off"
              error={registerValues.shopmobno.isValid}
              helperText={registerValues.shopmobno.errorMessage} />

            <TextField rows={4} multiline fullWidth size="small" label="Shop Address" variant="outlined" onChange={onChangeshopAddress} value={registerValues.shopaddress.text}
              autoComplete="off"
