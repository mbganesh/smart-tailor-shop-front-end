...
function RatesUpdater() {
  ...
  const onBtnClick = (buttonName, dress) => {
    ...
    if (dress === "pant" && buttonName === "Update") {
      ...
      if (temppanttypedkeys.length === pantcostkeys.length) {
        let pantupdateJson = {
          user: "admin", username: tokenData.userData.emailId,
          pantCost: pantcostlist
        };
        axios
          .post(APIClient.API_BASE_URL + "/rateProcess/insertPantCost", pantupdateJson,APIClient.API_HEADERS)
          .then((response) => {
    ...
    if (dress === "shirt" && buttonName === "Update") {
      ...
      if (tempshirttypedkeys.length === shirtcostkeys.length) {
        let shirtupdateJson = {
          user: "admin",
          shirtCost: shirtcostlist,username: tokenData.userData.emailId
        };

        axios
          .post(APIClient.API_BASE_URL + "/rateProcess/insertShirtCost", shirtupdateJson,APIClient.API_HEADERS)
          .then((response) => {
    ...
    if (dress === "blouse" && buttonName === "Update") {
      ...
      if (tempblousetypedkeys.length === blousecostkeys.length) {
        ...
        axios
          .post(APIClient.API_BASE_URL + "/rateProcess/insertBlouseCost", blouseupdateJson,APIClient.API_HEADERS)
          .then((response) => {
    ...
    if (dress === "salwar" && buttonName === "Update") {
      ...
      if (tempsalwartypedkeys.length === salwarcostkeys.length) {
        let salwarupdateJson = { user: "admin", username: tokenData.userData.emailId, salwarCost: salwarcostlist }
        axios
          .post(APIClient.API_BASE_URL + "/rateProcess/insertSalwarCost", salwarupdateJson,APIClient.API_HEADERS)
          .then((response) => {
  ...
  const onCancelBtnClick = (buttonName) => {
    onSalwarBlouseCostRate(tokenData);
  ...
  const onSalwarBlouseCostRate = (tokenData) => {
  ...
  useEffect(() => {
    ...
    onSalwarBlouseCostRate(decodedTokenData.message);
    ...
    <>
    {
      Object.keys(tokenData).length === 0 ?
        ...
        <div className={classes.root}>
          <div>
            <AppbarHead dataParent={{ appBtnColor: "rgba(220, 59, 81, 0.7)", appBtnText: "Rate Updater" , userData: tokenData.userData }} />
          </div>
